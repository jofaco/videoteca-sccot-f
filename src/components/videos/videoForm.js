...
const VideoForm = () => {
  ...
  const handleSubmit= async (e) => {
    ...
    try {
        let res;
        if (!params.id) {
          res = await VideoServer.RegisterVideo(video);
          const data = await res.json();
          if (data.message ==="Video agregado con exito!") {
            setVideo(initialState);
          }
        }
        else{
          await VideoServer.UpdateVideo(params.id, video);
        }        
        history("/");
    ...
    <div className="col-md-6 mx-auto">
      <form  onSubmit={handleSubmit} >
        ...
        {/* <div className="mb-3">
          ...
          <input className="form-control" id="featuredImage" name="featured_image" value={video.featured_image} onChange={handleInputChange} type="file" placeholder="Imagen para banner" required/>
